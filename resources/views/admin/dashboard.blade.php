...
<div class="min-h-screen bg-gray-100 flex flex-col" x-data="{ openSidebar: false }">
            ...
            <div class="flex gap-4 overflow-x-auto pb-2">
                @foreach ($lapaks as $lapak)
                    <div class="flex flex-col min-w-[220px] border rounded-lg shadow p-3"
                        ...
                        <h4 class="text-sm font-semibold text-center mb-2 bg-gray-100 p-2 rounded">{{ $lapak->nama_lapak }}</h4>
                        ...
                        <div class="rombong-container">
                            @if ($lapak->rombongs->count() > 0)
                                ...
                                @endphp
                                    <div class="flex flex-col items-center min-w-[120px] mb-3"
                                        draggable="true"
                                        ...
                                        <button class="{{ $warnaButton }} text-white text-sm font-medium rounded-lg px-3 py-2 shadow transition min-h-[40px] w-full text-center"
                                                ...
                                                data-lapak-id="{{ $lapak->lapak_id }}"
                                                onclick="showDetailKehadiran({{ $item->user_id }}, '{{ $item->user_name }}')">
                                        ...
                                        {{-- Waktu konfirmasi --}}
                                        @if($kehadiran && $kehadiran->waktu_konfirmasi)
                                            <div class="text-xs text-gray-500 mt-1">
                                                {{ $kehadiran->waktu_konfirmasi->format('H:i') }}
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
        ...
        <div class="mt-6 bg-white rounded-lg shadow p-4 mb-6">
            ...
            <ul class="text-sm text-gray-600 space-y-2">
                @forelse($pendingAnggota as $waiting)
                    <li class="flex justify-between items-center py-2">
                        <span>{{ $waiting->user->name }} ({{ $waiting->user->email }}) - Lapak: {{ $waiting->lapak->nama_lapak }}</span>
                        <div class="flex gap-2">
                            <form method="POST" action="{{ route('admin.anggota.approve', $waiting->waiting_list_id) }}">
                                @csrf
                                <button class="text-green-500 hover:text-green-700">
                                    <x-heroicon-o-check-circle class="h-6 w-6"/>
                                </button>
                            </form>
                            <form method="POST" action="{{ route('admin.anggota.reject', $waiting->waiting_list_id) }}">
                                @csrf
                                <button class="text-red-500 hover:text-red-700">
                                    <x-heroicon-o-x-circle class="h-6 w-6"/>
                                </button>
    ...
    <div id="modalDetailKehadiran" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
...
<script>
    ...
    fetch('/api/kehadiran/status')
        ...
        .then(data => {
            let countMasuk = 0, countLibur = 0, countMenunggu = 0, countStandby = 0;
            
            data.forEach(item => {
                const button = document.querySelector(`button[data-user-id="${item.user_id}"]`);
                if (button) {
                    // Update warna button
                    button.className = `${item.warna_button} text-white text-sm font-medium rounded-lg px-3 py-2 shadow transition min-h-[40px] w-full text-center`;
                    ...
                    const statusText = statusMap[item.status] || 'STANDBY';
                    ...
                    // Update title
                    button.title = `${item.nama} - ${statusText}`;
                    
                    // Add pulse animation if waiting
                    ...
                    // Count status
                    switch(item.status) {
                        case 'masuk': countMasuk++; break;
                        case 'libur': countLibur++; break;
                        case null: 
                            if (item.pesan_wa_terkirim) countMenunggu++;
                            else countStandby++;
                            break;
                        default: countStandby++;
                    }
                }
            });
            ...
            document.getElementById('countLibur').textContent = countLibur;
            document.getElementById('countMenunggu').textContent = countMenunggu;
            document.getElementById('countStandby').textContent = countStandby;
        })
...
function getStatusBadgeClass(status) {
    switch(status) {
        ...
        case 'libur': return 'bg-red-100 text-red-800';
